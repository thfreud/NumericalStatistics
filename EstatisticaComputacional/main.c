...
int main(void)
{
    ...
    clock_t begin = clock();
    int nRep = 10000;
    //vectors that will be used for saving data of each optimization
    ...
    for (int i = 0; i < nRep; i++) {
        ...
        if (status != GSL_SUCCESS) {
            ...
            // it is used an offset index for jumping the seed that had generated the last sample
            semente = *(uniform + (sample_size - 1) * j) * M;
            free(uniform);
            --i;
            /* here is generated a new vector of uniform random numbers 
            using the seed which had been chosen early at line number 112.*/ 
