...
int main(void)
{
    ...
    //printf("Entre com o tamanho de amostra: ");
    //scanf("%d", &sample_size);
    int nRep = 1000;
    ...
    for (int i = 0; i < nRep; i++) {
        ...
        if (status != GSL_SUCCESS) {
            printf("Semente: %lu\n", semente);
            // it gets the last seed which was generated by the reccurence relation in the congruential generator
            // it uses an offset index for jumping the seed the had generated the last sample
            semente = *(uniform + (sample_size - 1) * j) * M;
            printf("Semente: %lu\n", semente);
            free(uniform);
            --i;
            // it generates a new vector of uniform random numbers using as seed the seed at current i position 
    ...
    free(reg_scale);
    free(uniform);
    printf("\nExecuções de Monte Carlo: %d", taxa_sucesso);
    printf("\nTaxa de Falhas: %.4f%c", (double) 100*failsCount/nRep, '%');
    clock_t end = clock();
